package keqing.gtqtcore.network;
...
public enum GCNetworkManager {
    ...
    //当然也可以用别的，保证唯一即可。
    private final FMLEventChannel channel = NetworkRegistry.INSTANCE.newEventDrivenChannel(GTQTCore.MODID);
    ...
    private FMLProxyPacket createFMLProxyPacket(GCPacket pkt) {
        ...
        try {
            ...
            GTQTLog.logger.debug(e);
        }
        return new FMLProxyPacket(new PacketBuffer(buffer), GTQTCore.MODID);
    }
}