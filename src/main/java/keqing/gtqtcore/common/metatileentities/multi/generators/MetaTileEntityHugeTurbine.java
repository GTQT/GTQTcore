...
public class MetaTileEntityHugeTurbine extends FuelMultiblockController implements ITieredMetaTileEntity {
    ...
    public MetaTileEntityHugeTurbine(ResourceLocation metaTileEntityId, RecipeMap<?> recipeMap, int tier, IBlockState casingState, IBlockState gearboxState, ICubeRenderer casingRenderer, boolean hasMufflerHatch, ICubeRenderer frontOverlay) {
        ...
        this.tier = tier;
        this.recipeMapWorkable = new HugeTurbineWorkableHandler(this, tier);
        this.recipeMapWorkable.setMaximumOverclockVoltage(GTValues.V[tier]);
    }

    @Override
    public MetaTileEntity createMetaTileEntity(IGregTechTileEntity tileEntity) {
        return new MetaTileEntityHugeTurbine(metaTileEntityId, recipeMap, tier, casingState, gearboxState, casingRenderer, hasMufflerHatch, frontOverlay);
    }
    ...
    protected void formStructure(PatternMatchContext context) {
        ...
        ((HugeTurbineWorkableHandler) this.recipeMapWorkable).updateTanks();
    }

    @Override
    protected long getMaxVoltage() {
        long maxProduction = ((HugeTurbineWorkableHandler) recipeMapWorkable).getMaxVoltage();
        long currentProduction = ((HugeTurbineWorkableHandler) recipeMapWorkable).boostProduction((int) maxProduction);
        if (isActive() && currentProduction < maxProduction) {
            return ((HugeTurbineWorkableHandler) recipeMapWorkable).getMaxVoltage();
        } else {
            return 0L;
        }
    }
    ...
    protected void addDisplayText(List<ITextComponent> textList) {
        ...
        if (isStructureFormed()) {
            ...
            if (rotorHolder.getRotorEfficiency() > 0) {
                ...
                long maxProduction = ((HugeTurbineWorkableHandler) recipeMapWorkable).getMaxVoltage();
                long currentProduction = isActive() ? ((HugeTurbineWorkableHandler) recipeMapWorkable).boostProduction((int) maxProduction) : 0;
    ...
    public String[] getDescription() {
        return new String[]{I18n.format("gregtech.multiblock.huge_turbine.description")};
    }
