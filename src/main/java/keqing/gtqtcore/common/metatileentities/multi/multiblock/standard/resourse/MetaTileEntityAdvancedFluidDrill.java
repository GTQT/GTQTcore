...
import static keqing.gtqtcore.common.block.blocks.GTQTADVBlock.CasingType.*;
...
public class MetaTileEntityAdvancedFluidDrill extends MultiblockWithDisplayBase implements ITieredMetaTileEntity, IWorkable, IProgressBarMultiblock {
    ...
    private IBlockState getCasingState() {
        if (this.tier == 5) {
            return GTQTMetaBlocks.ADV_BLOCK.getState(MaragingSteel250);
        } else if (this.tier == 6) {
            return MetaBlocks.METAL_CASING.getState(BlockMetalCasing.MetalCasingType.HSSE_STURDY);
        } else {
            return  GTQTMetaBlocks.ADV_BLOCK.getState(this.tier == 7 ?IncoloyMA813:HastelloyX78);
        }
    }
