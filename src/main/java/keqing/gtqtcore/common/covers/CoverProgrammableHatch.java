...
import keqing.gtqtcore.common.items.behaviors.ProgrammableCircuit;
...
public class CoverProgrammableHatch extends CoverBase implements CoverWithUI, ITickable {
                ...
                for(int i=0;i<importItems.getSlots();i++)
                {
                    ...
                    if(itemStack!=ItemStack.EMPTY&&isItemValid(itemStack))
                    {
                        if(getProgrammableCircuit(itemStack).getName().equals("programmable_circuit")) {
                            ...
                            GhostCircuitItemStackHandler circuit = ReflectionHelper.getPrivateFieldValue(
                                    itemBus,
                                    "circuitInventory",
                                    GhostCircuitItemStackHandler.class
                            );

                            circuit.setCircuitValue(getProgrammableCircuit(itemStack).getType());
    ...
    public ProgrammableCircuit getProgrammableCircuit(@Nonnull ItemStack stack)
    {
