...
public class EvapRecipeLogic extends MultiblockRecipeLogic {
    ...
    protected void updateRecipeProgress() {
        ...
        if (pool.coilStats != null && pool.isHeated()) {
            ...
            int electricEnergy = coilHeat * (((pool.getColumnCount() / 2 + 1) * pool.getRowCount()) + pool.getColumnCount() / 2) / 10; // converted to EU to save on divisions
            if (electricEnergy > pool.getEnergyContainer().getEnergyStored()) electricEnergy = (int) pool.getEnergyContainer().getEnergyStored();
            boolean couldInput = pool.inputEnergy(electricEnergy * 10); // if room is available in thermal energy storage
            if (couldInput) {
