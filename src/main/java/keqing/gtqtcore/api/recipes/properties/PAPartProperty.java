...
public class PAPartProperty extends RecipeProperty<Integer> {
    public static final String KEY = "Part";

    private static final TreeMap<Integer, String> registeredPart = new TreeMap<>();
    private static PAPartProperty INSTANCE;

    private PAPartProperty() {
        super(KEY, Integer.class);
    }

    @Override
    public void drawInfo(Minecraft minecraft, int x, int y, int color, Object value) {
        minecraft.fontRenderer.drawString(I18n.format("密钥：",
                castValue(value).toString()) + getPart(castValue(value)), x, y, color);
    }

    public static PAPartProperty getInstance() {
        if (INSTANCE == null) {
            INSTANCE = new PAPartProperty();
        }
        return INSTANCE;
    }
