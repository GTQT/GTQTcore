...
public abstract class mixinGCYMMultiblockRecipeLogic  extends MultiblockRecipeLogic {
   ...
   public void setMaxProgress(int maxProgress)
    {
        if(((IParallelMultiblock)this.metaTileEntity).getMaxParallel()==16)this.maxProgressTime = maxProgress;
        else if (((IParallelMultiblock) this.metaTileEntity).getMaxParallel() <= 16)
